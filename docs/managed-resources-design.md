...
**3. Uploaded bytes are served from a different origin** — `sw5e-files.cfowers.io`,
not `sw5e.cfowers.io`. The session cookies are host-only: nothing in the
identity configuration sets `Cookie.Domain`, so ASP.NET Core issues them scoped
to the exact host, and a sibling subdomain cannot receive them even if
something on it were made to run. This is why a second registrable domain is
...
**security event**, recorded and surfaced to administrators — not a silent
success.
...
The output is inert by construction — it is pictures in a PDF wrapper, with no
...
look at — not a flag the uploader sets.
